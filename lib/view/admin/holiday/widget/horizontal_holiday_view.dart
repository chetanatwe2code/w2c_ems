...
class HorizontalHolidayView extends StatelessWidget {
  final EventModel? model;
  final bool isFirst;
  const HorizontalHolidayView({super.key,this.model , this.isFirst = false});
  ...
  Widget build(BuildContext context) {
   ...
   String img = model?.eventType == "event" ? appEvent : appHoliday;
    return Stack(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      if(model?.endDate?.isNotEmpty??false)
                        Flexible(
                          child: Text(" - ${(model?.endDate??"").toDateDMMMYY()}",
                            style: TextStyle(
                                fontSize: fontSize2,
                                color: textColor
                            ),),
